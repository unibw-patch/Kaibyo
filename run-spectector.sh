...
do
    ...
    do
        ...
        log=output/logs/spectector/$version-cloop.$mitigation.o0.log
        timeout $timeout spectector benchmarks/spectre/asm/$version-cloop.$mitigation.o0.s -e [victim_function_$version] > $log
        tail -n 1 "$log" | grep "program is"
        echo Running $version-cloop.$mitigation.o2.s
        log=output/logs/spectector/$version-cloop.$mitigation.o2.log
        timeout $timeout spectector benchmarks/spectre/asm/$version-cloop.$mitigation.o2.s -e [victim_function_$version] > $log
        tail -n 1 "$log" | grep "program is"
        echo =========================================================
        echo Running $version-sloop.$mitigation.o0.s
        log=output/logs/spectector/$version-sloop.$mitigation.o0.log
        timeout $timeout spectector benchmarks/spectre/asm/$version-sloop.$mitigation.o0.s -e [victim_function_$version] > $log
        tail -n 1 "$log" | grep "program is"
        echo Running $version-sloop.$mitigation.o2.s
        log=output/logs/spectector/$version-sloop.$mitigation.o2.log
        timeout $timeout spectector benchmarks/spectre/asm/$version-sloop.$mitigation.o2.s -e [victim_function_$version] > $log
        tail -n 1 "$log" | grep "program is"
    done
    echo =========================================================
    echo
