...
void victim_function_v02(size_t x) {
    for (int j = 1; j <= x; ++j) {
        if (j < array1_size) {
            leakByteLocalFunction(array1[j]);
        }
    }
}
#endif
...
void victim_function_v03(size_t x) {
    for (int j = 1; j <= x; ++j) {
        if (j < array1_size)
            leakByteNoinlineFunction(array1[j]);
    }
}
#endif
...
#ifdef v05
// ----------------------------------------------------------------------------------------
// EXAMPLE 5:  Use x as the initial value in a for() x.
//
// Comments: Output is unsafe.
// ----------------------------------------------------------------------------------------
...
void victim_function_v11(size_t x) {
    for (int j = 1; j <= x; ++j) {
        if (j < array1_size)
            temp = mymemcmp(&temp, array2 + (array1[j] * SIZE), 1);
    }
}
#endif
