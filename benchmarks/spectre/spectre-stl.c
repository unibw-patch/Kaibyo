...
void victim_function_v5(uint32_t idx) {  // INSECURE
  ...
  uint32_t toleak = case5_ptr[ridx];
...
void victim_function_v6(uint32_t idx) { // INSECURE
  ...
  uint32_t toleak = (case6_array[case6_idx])[ridx];
  temp &= publicarray2[toleak];
}
#endif
...
void victim_function_v8(uint32_t idx) {  // INSECURE
  ...
  uint32_t toleak = publicarray[idx * case8_mult];
  temp &= publicarray2[toleak];
}
#endif
...
void victim_function_v11(uint32_t idx) { // INSECURE
  uint32_t to_leak = case_11_load_value(idx);
...
void victim_function_v13(uint32_t idx) {  // SECURE
  register uint32_t to_leak asm ("edx");
